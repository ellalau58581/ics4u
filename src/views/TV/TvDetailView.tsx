...
export const TvDetailView = () => {
  const navigate = useNavigate();
  const { id } = useParams();
  const { data } = useTmdb<TvShowResponse>(`${TV_ENDPOINT}/${id}`, {
    append_to_response: 'videos',
  });

  const trailerVideo = data?.videos?.results.find((video) => video.site === 'YouTube' && video.type === 'Trailer');
