import { DetailItem, ImageGrid, Modal, Pagination } from '@/components';
...
export const App = () => {
  ...
  useEffect(() => {
    const fetchMovies = async () => {
      try {
        const response = await axios.get<NowPlayingResponse>(NOW_PLAYING_ENDPOINT, {
          params: {
  ...
  const fetchMovie = async (id: number) => {
    try {
      const response = await axios.get<MovieResponse>(`${MOVIE_ENDPOINT}/${id}`, {
        params: {
          api_key: import.meta.env.VITE_TMDB_API_KEY,
        },
      });
  ...
  return (
    <main className="flex min-h-screen flex-col border-b border-gray-700 bg-gray-900 text-white">
      <section className="mx-auto max-w-7xl space-y-5 p-5">
        ...
        <Pagination page={page} maxPages={movies.total_pages} onClick={setPage} />
      </section>
      {movie && (
